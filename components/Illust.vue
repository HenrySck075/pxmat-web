<template>
  <div>
    <v-card elevation="2" width="200px" :class="!compact?'ma-2':'ma-1'">
      <NuxtLink :to="'/artworks/'+data.id" :custom=!interact class="uncolored-anchor">
        <v-img :src=proxyAssetUrl(data.url) max-height="400px" :style="{position: 'relative', opacity: imgOpacity}" @mouseover="imgOpacity = 0.6" @mouseleave="imgOpacity = 1">
          ...
          <div style="font-size: 10px; position: absolute; top:5px; right:12px; background-color: rgba(133,133,133,0.5)" v-if="data.pageCount>1" class="pa-1 font-weight-bold">
            <v-icon size="10px" icon="mdi-layers-outline"></v-icon>
            {{data.pageCount}}
          </div>
